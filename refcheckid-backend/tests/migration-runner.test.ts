...
describe('database migration runner', () => {
  it('lists canonical SQL migrations from database/migrations with checksums', () => {
    ...
    expect(output).toContain('Migration count: 21');
    expect(output).toContain('0021_add_match_sheet_player_roles.sql');
  });
  ...
  it('validates migrations in dry-run mode without requiring database credentials', () => {
    ...
    expect(output).toContain('Dry run: 21 migrations are valid and ready for Supabase CLI.');
  });
